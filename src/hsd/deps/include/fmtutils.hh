...
template<class B>
static string readLine(B& in) {
  string rstr;
  skipWhitespace(in);
  for(;;) {
    if (*in == '\n') { ++in; return rstr; }
    if (*in == '\r') { ++in; return rstr; }
    rstr.push_back(*in); ++in;
  }
  return NULL;
}
...
static double parseDouble(B& in) {
	...
	++in;
      ...
      NDBG(std::cout << "VAL: " << val << endl; cout.flush(););
      val = val / ndec;
    }
    return neg ? -val : val; }
