...
inline void GEN_PRINT_ELEMENTS (ST& outs, const T& coll, const char* istr="",
				const char* sstr=" ", const char* estr="\n")
{
  outs << istr;
  typename T::const_iterator pos = coll.begin();
  if (pos == coll.end()) { outs << estr; return; }
  while (true) {
    outs << *(pos++);
    if (pos != coll.end()) { outs << sstr; }
    else                   { break; }
  }
  outs << estr;
}
