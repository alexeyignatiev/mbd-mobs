...
#ifndef CLSET_EXT_HH_
#define CLSET_EXT_HH_
...
public:
	...
	void erase_subset(size_t start, ClauseSet& clset)
	{
		if (clset.size()) {
			...
			auto begin = std::find(clauses.begin() + start, clauses.end(), cl_first);
	...
	void add_clause(vector<long>& cl)
	{
		vector<int> cl_temp(cl.size());

		for (size_t i = 0; i < cl.size(); ++i)
			cl_temp[i] = cl[i];

		clauses.push_back(cl_temp);
	}
